    ...
    <section className="w-full my-20" id="styles">
      ...
      <div className="flex gap-3 overflow-x-auto lg:hidden snap-x snap-mandatory px-3">
        {imgs.map((item, i) => (
          <div
            key={i}
            className="relative snap-start shrink-0 w-2/3 xs:w-1/2 sm:w-1/3 aspect-[2/3] bg-center bg-cover rounded-lg"
            style={{ backgroundImage: `url('${item.src}')` }} // ← antes usabas `src` (objeto)
            aria-label={item.name}
            role="img"
          >
            <p className="absolute bottom-1/2 left-1/2 -translate-x-1/2 rotate-270 text-4xl uppercase">
              {item.name}
            </p>
          </div>
        ))}
      ...
      <div className="hidden lg:grid grid-cols-7 w-full px-3">
        {imgs.map((item, i) => (
          ...
          >
            <p className="absolute bottom-1/2 left-1/2 -translate-x-1/2 rotate-270 text-6xl uppercase">
              {item.name}
            </p>
          </div>
        ))}
